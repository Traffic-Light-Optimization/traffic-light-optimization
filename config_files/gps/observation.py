...
class ModelGpsObservationFunction(ObservationFunction):
    ...
    def __call__(self) -> np.ndarray:
        ...
        # print()
        observation = np.array(phase_id + queue + occupancy + avg_speeds + wait_times + density, dtype=np.float32)
        return observation

    def observation_space(self) -> spaces.Box:
        """Return the observation space."""
        return spaces.Box(
            low=np.zeros(self.ts.num_green_phases + 5*len(self.ts.lanes) + self.ts.num_green_phases, dtype=np.float32),
            high=np.ones(self.ts.num_green_phases + 5*len(self.ts.lanes) + self.ts.num_green_phases, dtype=np.float32),
        )
