...
class ModelGpsObservationFunction(ObservationFunction):
    ...
    def __call__(self) -> np.ndarray:
        ...
        print()
        observation = np.array(density + queue + occupancy + wait_times + avg_speeds + phase_times, dtype=np.float32)
        return observation

    def observation_space(self) -> spaces.Box:
        """Return the observation space."""
        return spaces.Box(
            low=np.zeros(5*len(self.ts.lanes) + self.ts.num_green_phases, dtype=np.float32),
            high=np.ones(5*len(self.ts.lanes) + self.ts.num_green_phases, dtype=np.float32),
        )
